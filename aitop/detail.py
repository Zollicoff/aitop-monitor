...
class AgentSummary(Static):
    ...
    def render(self) -> str:
        ...
        for label, attr in [
            ...
            for tf in TIMEFRAMES:
                _, tc = s.usage_for(tf)
                row += f" {fmt_cost(getattr(tc, attr)):>8}"
            lines.append(row)
...
class ProjectBreakdown(Static):
    ...
    def render(self) -> str:
        projects = self._session.usage_by_project("all")
        if not projects:
            return "  [dim]No project data[/]"

        max_cost = max((c.total for _, c in projects.values()), default=1) or 0.01
        lines = []
        lines.append(
        ...
        for cwd, (tokens, cost) in projects.items():
            path = short_path(cwd)
            if len(path) > 38:
                path = "…" + path[-(37):]
...
class AgentDetailScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="detail-container"):
            ...
            yield AgentSummary(self._session)
            yield Static("")
            yield Static("[bold]COST BY PROJECT (all time)[/]", classes="panel-title")
            yield ProjectBreakdown(self._session)
        yield Footer()
